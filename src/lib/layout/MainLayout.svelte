<script lang="ts">
  ...
  const { activeProject, onProjectChange, headerChildren, leftPanel, middlePanel, rightPanel }: Props = $props();
...
<div class="h-screen flex flex-col overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-black text-slate-200">
    ...
    {#if isMobile}
      ...
      <div class="flex-1 flex flex-col min-h-0">
          ...
          <div class="glass-header border-t border-slate-700/50 flex justify-around p-2 shrink-0">
              <button
                class="p-2 rounded-lg {activeMobileTab === 'left' ? 'bg-blue-500/20 text-blue-400' : 'text-slate-400'}"
                onclick={() => activeMobileTab = 'left'}
              >
                Benchmarks
              </button>
              <button
                class="p-2 rounded-lg {activeMobileTab === 'middle' ? 'bg-blue-500/20 text-blue-400' : 'text-slate-400'}"
                onclick={() => activeMobileTab = 'middle'}
              >
                Jobs
              </button>
              <button
                 class="p-2 rounded-lg {activeMobileTab === 'right' ? 'bg-blue-500/20 text-blue-400' : 'text-slate-400'}"
