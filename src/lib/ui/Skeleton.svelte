<script lang="ts">
  ...
  const finalWidth = $derived(
    (width?.startsWith('w-') ?? false)
      ? width
      : (width === undefined || width === '' ? defaultDimensions[variant].width : '')
  );

  const finalHeight = $derived(
    (height?.startsWith('h-') ?? false)
      ? height
      : (height === undefined || height === '' ? defaultDimensions[variant].height : '')
  );
</script>

<div
  class="animate-pulse bg-slate-800/50 {variantClasses[variant]} {finalWidth} {finalHeight} {className}"
  {style}
></div>
